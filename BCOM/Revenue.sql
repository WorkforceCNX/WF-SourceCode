...
COALESCE(ActualRevenue.[Date],Budget.[Date],NewHire.[Date])                                AS [Date],
COALESCE(ActualRevenue.[LOB],Budget.[LOB],NewHire.[LOB])                                   AS [LOB],
SUM(ISNULL(ActualRevenue.[ACT_Night_Productive],0)                                *11.96)  AS [ACT_Night_Prod],
SUM(ISNULL(ActualRevenue.[ACT_Day_Productive]-ActualRevenue.[ACT_OTRegistered],0) *08.86)  AS [ACT_Day_Prod],
SUM(ISNULL(ActualRevenue.[ACT_Night_Downtime],0)                                  *11.96)  AS [ACT_Night_Downtime],
SUM(ISNULL(ActualRevenue.[ACT_Day_Downtime],0)                                    *08.86)  AS [ACT_Day_Downtime],
SUM(ISNULL(ActualRevenue.[ACT_NewHire_Training],0)                                *08.86)  AS [ACT_NewHireTrain],
SUM(ISNULL(ActualRevenue.[ACT_PH],0)                                              *11.96)  AS [ACT_PH],
SUM(ISNULL(ActualRevenue.[ACT_OTRegistered],0)                                    *13.29)  AS [ACT_OTRegis],
SUM(ISNULL(Budget.[Night_Prod_req],0)                                             *11.96)  AS [REQ_Night_Prod],
SUM(ISNULL(Budget.[Day_Prod_req],0)                                               *08.86)  AS [REQ_Day_Prod],
SUM(ISNULL(Budget.[Night_Downtime_req],0)                                         *11.96)  AS [REQ_Night_Downtime],
SUM(ISNULL(Budget.[Day_Downtime_req],0)                                           *08.86)  AS [REQ_Day_Downtime],
SUM(ISNULL(Budget.[PH_req],0)                                                     *11.96)  AS [REQ_PH],
SUM(ISNULL(NewHire.[Newhire_Req],0)                                               *08.86)  AS [REQ_NewHire],
'Revenue (€)'                                                                              AS [Type]
From ActualRevenue 
Full Join Budget On ActualRevenue.[LOB] = Budget.[LOB] And ActualRevenue.[Date] = Budget.[Date] 
Full Join NewHire On ActualRevenue.[LOB] = NewHire.[LOB] And ActualRevenue.[Date] = NewHire.[Date]
