CREATE OR ALTER PROCEDURE BCOM.Refresh_Schedule_Data
AS
BEGIN
    SET NOCOUNT ON;
	PRINT 'Starting procedure BCOM.Refresh_Schedule_Data.';
    PRINT 'Truncating table BCOM.Schedule...';
    TRUNCATE TABLE BCOM.Schedule; --Clear Schedule
	PRINT 'Table BCOM.Schedule truncated successfully. And start run Schedule scrip';
/*                                                           
----------------------------------------------------------------------------------------------------------------------------------
--                                       |                                       |                                              --
--                                       |            IMPORT CODE HERE           |                                              --
...
*/
	...
	CASE 
		...
		WHEN ROSTER_RAW2.[LOB] IN ('NL', 'IT', 'FR', 'ES') THEN 'Unbabel Focus'
		WHEN ROSTER_RAW2.[LOB] IN ('FR CSP', 'ES CSP', 'IT CSP', 'DE CSP', 'VI TRA CSP', 'ZH CSP', 'PT CSP', 'NO CSP', 'HR CSP', 'EL CSP', 'PL CSP', 'CS CSP', 'SV CSP', 'UB CSP') THEN 'Unbabel CSP'
...
----------------------------------------------------------------------------------------------------------------------------------
*/
    INSERT INTO BCOM.Schedule (  -- Insert data to BCOM.Schedule 
		[Shift], [Shift_type], [Original_Shift], [LOB], [week_shift], [week_off], [TL_ID], [TL_Name], [OM_ID], [OM_Name], [DPE_ID], [DPE_Name], [Emp ID], [Emp_Name], [Wave], [Booking Login ID], [TED Name], [cnx_email], 
		[Booking Email], [CUIC Name], [PST_Start_Date], [Date], [Tenure], [Tenure days], [Week_num], [Shift_definition], [YEAR], [MONTH], [Week_day], [Termination/Transfer], [LOB Group], [ScheduleSeconds(s)], [Work Type], [Site]
    )
    SELECT  -- Get Data from final CTE (ROSTER_RAW3)
		[Shift], [Shift_type], [Original_Shift], [LOB], [week_shift], [week_off], [TL_ID], [TL_Name], [OM_ID], [OM_Name], [DPE_ID], [DPE_Name], [Emp ID], [Emp_Name], [Wave], [Booking Login ID], [TED Name], [cnx_email], 
		[Booking Email], [CUIC Name], [PST_Start_Date], [Date], [Tenure], [Tenure days], [Week_num], [Shift_definition], [YEAR], [MONTH], [Week_day], [Termination/Transfer], [LOB Group], [ScheduleSeconds(s)], [Work Type], [Site]
    FROM ROSTER_RAW3; 
    RAISERROR('Data insertion into BCOM.Schedule completed.', 0, 1) WITH NOWAIT;
    PRINT 'Fetching top 5 rows from BCOM.Schedule as sample data...';
	--Overview Sample Data
	SELECT TOP 5 * FROM BCOM.Schedule;
    PRINT 'Procedure BCOM.Refresh_Schedule_Data completed successfully.';
    SET NOCOUNT OFF;
END;
GO
...
-- EXEC BCOM.Refresh_Schedule_Data;