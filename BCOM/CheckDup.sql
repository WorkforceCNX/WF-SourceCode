...
-- 2. BCOM.AHT
AHT_Raw1 as (
Select cast([Date] AS varchar)+[Agent Name Display]+[Answered Language Name]+[Measure Names]+cast([Measure Values] as varchar) as [Concat], COUNT(*) as [Count]
From BCOM.AHT2
Where cast([Date] AS varchar)+[Agent Name Display]+[Answered Language Name]+[Measure Names]+cast([Measure Values] as varchar) Is not Null
Group by cast([Date] AS varchar)+[Agent Name Display]+[Answered Language Name]+[Measure Names]+cast([Measure Values] as varchar) Having COUNT(*)>1
),
-- 3. BCOM.CapHC
CapacityHC_Raw1 as (
